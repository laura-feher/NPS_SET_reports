...
The long-term relative rate of sea-level rise was obtained from the [NOAA tides and currents website](https://tidesandcurrents.noaa.gov/sltrends/) for the nearby tide gauge at `r` . This long-term SLR rate is based on the full record of water level observations collected between the year that the gauge was installed (`r min(slr_data$Year`) to the previous year (`r year(Sys.Date())-1`).
...
### 2. How do local wetland elevation dynamics relate to the local hydrology (e.g., local tidal datums)?
...
```{r site-slr-recent-fig, echo=FALSE, message=FALSE, warning=FALSE, out.width='100%', fig.cap="Site-level surface elevation change at ", params$park_name, " compared to relative long-term and recent rates of sea-level rise. ... Hover the mouse over a data point to see the cumulative surface elevation change value on a specific date, or click and drag over a specific time to zoom in. Double click to zoom back out"}
